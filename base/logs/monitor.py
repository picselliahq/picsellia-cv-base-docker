...
class LogMonitor:
    ...
    def append_to_buffer(self, line: str, section_header: str):
        ...
        self.logs[section_header]["logs"][str(self.job.line_nb + len(self.buffer))] = (
            line
        )
    ...
    def end_log_monitoring(self, exit_code: int, section_header: str):
        ...
        Returns:
            None
        """
        with open(f"{self.job.id}-logs.json", "w") as json_log_file:
            if self.buffer:
        ...
        self.job.store_logging_file(f"{self.job.id}-logs.json")
